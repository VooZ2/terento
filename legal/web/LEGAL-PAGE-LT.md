# Teisinė informacija

Šis puslapis paaiškina, kas yra Terento ir kas nėra. Tai vieši pranešimai apie svetainę ir projektą.

Tai nėra programos vartotojo sutartis, ne App Store EULA, ne Garmin dokumentas ir ne skaitmeninio turinio sutartis pagal Direktyvą (ES) 2019/770.

Svetainė, kuriama macOS programa ir žemėlapių failai yra skirtingi dalykai. Žemėlapiai nėra Terento kūrinys.

**Būsena.** Projektą kaip privatus asmuo skelbia Gediminas. Privatumo
klausimams: [privacy@terento.app](mailto:privacy@terento.app).

## Garmin

Garmin priklauso Garmin Ltd. Terento nėra susijęs su Garmin ir nėra jo remiamas, sertifikuotas ar partneris.

Garmin vardus (įskaitant produktų pavadinimus) vartojame tik tam, kad būtų aišku, su kokiais laikrodžiais dirbame: suderinamumui, bandymams, dokumentacijai ir paieškai. Garmin logotipų nenaudojame.

## Apple ir macOS

Apple, Mac, macOS ir Apple Silicon priklauso Apple Inc. Terento nėra susijęs su Apple. Šie vardai reikalingi, nes programa kuriama „Mac“ kompiuteriui.

## Žemėlapiai

Terento žemėlapių nekuria, neperlicencijuoja ir nevadina „Terento žemėlapiais“.

Freizeitkarte, OpenTopoMap, OpenStreetMap ir kiti duomenų teikėjai lieka savo licencijų savininkais. Terento tik sujungia kelią: atsisiuntimas iš pirminio teikėjo → jūsų „Mac“ → laikrodis. Savo serveriais žemėlapių failų nehostiname ir neplatiname.

Daugiau: [Freizeitkarte](https://www.freizeitkarte-osm.de/), [OpenTopoMap](https://garmin.opentopomap.org/), [OpenStreetMap autorių teisės](https://www.openstreetmap.org/copyright).

## Programos licencija

Terento išeities kodas licencijuojamas pagal GNU GPL 3.0 arba vėlesnę versiją. Pilnas tekstas: [LICENSE](https://github.com/VooZ2/terento) saugykloje.

GPL suteikia laisves kodui. Ji nesuteikia teisės naudoti pavadinimą ar ženklą „Terento“ kaip prekių ženklą ir neleidžia teigti, kad pakeista versija yra oficialus Terento leidimas, jei taip nėra.

Šios svetainės HTML, CSS ir JavaScript, kurių kilmė yra Terento, taip pat GPL, nebent faile nurodyta kitaip. Šriftai ir kitos trečiųjų šalių dalys lieka pagal savo licencijas — žr. [THIRD_PARTY_NOTICES.md](https://github.com/VooZ2/terento/blob/main/THIRD_PARTY_NOTICES.md).

## Kitos programos

Kad macOS programa galėtų kalbėti su laikrodžiu per MTP, kūrimo versija naudoja Homebrew bibliotekas. Produkcinė programa dinamines bibliotekas perstato iš prisegtų pirminių šaltinių ir įdeda į \`Terento.app/Contents/Frameworks\`:

- **libmtp** — LGPL-2.1-or-later, [github.com/libmtp/libmtp](https://github.com/libmtp/libmtp)
- **libusb** — LGPL-2.1-or-later, [github.com/libusb/libusb](https://github.com/libusb/libusb)

Kiekviena biblioteka lieka pagal savo licenciją. Terento jų neperlicencijuoja.

## Be garantijos

Programa platinama su viltimi, kad bus naudinga, bet **be garantijos**. Terento neatsako, jei naudodamiesi projektu, svetaine ar programa įrenginys nustos veikti, prarasite duomenis arba žemėlapiai neveiks.

Produkto taisyklė: verčiau sustoti, nei spėlioti ir gadinti. Tai nėra pažadas, kad niekas nesuges.

Nieko čia parašyto negalima suprasti kaip atsisakymo teisių, kurių pagal taikomą teisę atsisakyti negalima — įskaitant ES ir Lietuvos vartotojų apsaugą.

## Kas veikia svetainėje ir programoje

Viešą svetainę pristato ir saugo **Cloudflare**. Lankomumą skaičiuoja
**Umami** (be slapukų ir be asmeninio profiliavimo). Kalbos pasirinkimas gali
būti įrašytas jūsų naršyklėje.

Naujo macOS beta diegimo metu suderinamumo duomenų dalijimosi pasirinkimas
rodomas prieš diegimą ir pažymimas pagal nutylėjimą. Jei naudotojas jį palieka
įjungtą ir tęsia diegimą, siunčiama tik privatumo prasme minimali suderinamumo
ataskaita. Pasirinkimą galima atžymėti; tai neblokuoja diegimo ir nemažina
programos funkcionalumo. Išsami informacija pateikta [Privatumo puslapyje](/privacy/).

Išsamiau: [Privatumas](/privacy/).

## Ko dar nėra

Nėra registruoto juridinio asmens, įmonės kodo ar PVM kodo. Privatumo
klausimams naudojamas [privacy@terento.app](mailto:privacy@terento.app). Jei
siūloma savanoriška parama, ji neatrakina funkcijų ar žemėlapių.

Šis tekstas yra projekto pranešimas, ne teisinė konsultacija.
