# Právní informace

Terento je bezplatná aplikace macOS s otevřeným zdrojovým kódem pro instalaci a správu map třetích stran na hodinkách Garmin. Tyto informace se vztahují na aplikaci a tento web.

## Projekt

Terento je nezávislý projekt vydávaný soukromou osobou. Tvůrce najdete na stránce [O projektu](/cs/about/). Kontakt: [hello@terento.app](mailto:hello@terento.app).

Aplikace je veřejná beta pro Macy s Apple Silicon. Kompatibilitu hodnotíme pro každý přesný model a variantu hodinek. Před použitím ověřte [kompatibilitu](/cs/compatibility/). Příspěvky jsou dobrovolné a neodemykají funkce, mapy ani poskytovatele.

## Licence softwaru a dalších součástí

Zdrojový kód Terento a vlastní HTML, CSS a JavaScript webu jsou pod **GPL-3.0-or-later**, pokud soubor neuvádí jinak. Přečtěte si [licenci](https://github.com/VooZ2/terento/blob/beta/LICENSE).

Přibalené knihovny si ponechávají vlastní licence: **libmtp 1.1.23** a **libusb 1.0.30** používají LGPL-2.1-or-later. Pro písma a další obsah třetích stran platí jejich podmínky. Licence a informace o zdrojích jsou v [oznámeních třetích stran](https://github.com/VooZ2/terento/blob/beta/THIRD_PARTY_NOTICES.md).

Softwarová licence neposkytuje práva k ochranné známce Terento. Upravené verze nesmějí být prezentovány jako oficiální vydání Terento.

## Licence map

Mapy pocházejí od [MapRando](https://ravenfeld.gitlab.io/open-garmin-map/), [Freizeitkarte](https://www.freizeitkarte-osm.de/) a [OpenTopoMap](https://garmin.opentopomap.org/) a podléhají jejich licencím a [uvedení autorství OpenStreetMap](https://www.openstreetmap.org/copyright). Pro vlastní importované mapy nadále platí příslušné podmínky.

Terento tyto mapy nevytváří, nemění jejich licence ani jejich soubory nehostuje, nezrcadlí či nešíří na svých serverech. Katalogové mapy se stahují od původního poskytovatele na váš Mac a pak do hodinek.

## Názvy a nezávislost

Garmin a názvy jeho produktů jsou ochranné známky Garmin Ltd. Apple, Mac, macOS a Apple Silicon jsou ochranné známky Apple Inc. Používají se popisně. Terento není s těmito společnostmi spojeno ani jimi podporováno nebo certifikováno.

## Beta a vaše práva

Beta může obsahovat chyby ovlivňující mapy, fungování zařízení nebo data. V rozsahu povoleném právem je software poskytován bez záruky, jak uvádí jeho licence.

Žádné oznámení nevylučuje ani neomezuje odpovědnost tam, kde to zákon zakazuje, včetně úmyslu nebo hrubé nedbalosti. Vaše kogentní práva podle litevského práva a práva EU zůstávají nedotčena.

## Soukromí

Informace o zpracování dat, diagnostice a statistikách webu najdete v [oznámení o soukromí](/privacy/).

Aktualizováno: 5. září 2026.
