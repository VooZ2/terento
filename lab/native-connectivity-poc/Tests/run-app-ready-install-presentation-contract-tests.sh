...
require_text 'Spacer(minLength: TerentoPageLayout.sectionSpacing)' 'sharing block is not separated with flexible space'
require_text 'Share anonymous installation results to help improve Garmin compatibility.' 'approved sharing copy is missing'
require_text 'Link("What is shared and how to stop sharing ↗", destination: TerentoAppLinks.privacy)' 'external privacy link wording or indicator changed'
require_text '.toggleStyle(.checkbox)' 'sharing control is not a native checkbox'
require_text '.tint(TerentoColors.interactive)' 'sharing checkbox does not use the interactive brand tint'
require_text '.frame(maxWidth: 740, alignment: .leading)' 'sharing block is not kept compact and left aligned'
