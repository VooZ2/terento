-- Statistics semantics v1: one independent main-map result per
-- operation_id + map_result_index. Do not rewrite retained evidence.
-- Missing current write facts remain outside fresh-install denominators;
-- only legacy rows without the v3 diagnostic fields retain their historical
-- attempted-write interpretation.
ALTER TABLE compatibility_evidence_event
    ADD COLUMN optional_component_selected BOOLEAN,
    ADD COLUMN optional_component_outcome TEXT CHECK (
        optional_component_outcome IN ('VERIFIED', 'FAILED', 'NOT_STARTED', 'UNKNOWN')
    ),
    ADD COLUMN optional_component_failure_stage TEXT CHECK (
        optional_component_failure_stage IN ('download', 'extract', 'source-validation',
            'preflight', 'write', 'verify', 'cleanup', 'manifest')
    ),
    ADD COLUMN optional_component_failure_code TEXT,
    ADD COLUMN optional_component_native_failure_code TEXT;
