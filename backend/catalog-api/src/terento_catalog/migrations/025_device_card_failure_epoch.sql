-- Restore the main compatibility/Installations aggregate to active,
-- write-started evidence. Device cards use a separate deployment-time epoch:
-- successful history remains visible, while only failures received after this
-- migration contribute to the card's Attempts and Failed counters.

CREATE TABLE compatibility_device_card_failure_epoch (
    singleton BOOLEAN PRIMARY KEY DEFAULT TRUE CHECK (singleton),
    starts_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

INSERT INTO compatibility_device_card_failure_epoch (singleton)
VALUES (TRUE)
ON CONFLICT (singleton) DO NOTHING;

UPDATE compatibility_evidence_event
SET resolution_note = 'Historical pre-beta.6 failure; excluded from current compatibility statistics.'
WHERE resolution_code = 'LEGACY_PRE_BETA6'
  AND resolution_note = 'Historical pre-beta.6 failure; retained in all compatibility counts and rates.';
