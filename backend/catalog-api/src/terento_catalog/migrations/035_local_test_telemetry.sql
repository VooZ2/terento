-- Keep local release validation separate from production statistics.
-- A row is local-test data only when the server derives this flag from the
-- explicit release-label contract; missing or production labels remain false.
...
WHERE release_label ~* '^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?-local$';

-- Older map rows did not carry a release label. Link only to an already
-- classified compatibility operation; unmarked map-only history is retained.
UPDATE map_download_event AS m
SET is_local_test = TRUE,
    release_label = e.release_label
FROM (
    SELECT operation_id, min(release_label) AS release_label
    FROM compatibility_evidence_event
    WHERE is_local_test IS TRUE
      AND operation_id IS NOT NULL
    GROUP BY operation_id
) AS e
WHERE m.operation_id = e.operation_id;

CREATE INDEX compatibility_evidence_local_test_idx
    ON compatibility_evidence_event(is_local_test, occurred_at DESC);

CREATE INDEX map_download_event_local_test_idx
    ON map_download_event(is_local_test, occurred_at DESC);
