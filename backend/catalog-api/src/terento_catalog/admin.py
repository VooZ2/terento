    ...
    return r"""(() => {
      ...
      const showWorldMapTooltip = (item, code) => {
        ...
        const providersMarkup = item?.providers
          ...
          : '';
        worldMapTooltip.innerHTML = `<strong>${escapeHtml(label)}</strong><span class="world-map-tooltip-total">${item?.count || 0} completed installs</span>${providersMarkup || '<span class="world-map-tooltip-empty">No recorded installs</span>'}`;
      ...
      const renderWorldMap = (installRows) => {
        ...
        regions.forEach((path) => {
          ...
          path.style.fill = item && maximum ? `hsl(198 25% ${lightness}%)` : '#fff';
          path.setAttribute('tabindex', '0');
          path.setAttribute('role', 'button');
          path.setAttribute('aria-label', `${countryNames[code] || code.toUpperCase()}: ${item?.count || 0} completed installs`);
        ...
        if (worldMapStatus) worldMapStatus.textContent = maximum ? `${items.length} countries · ${items.reduce((total, item) => total + item.count, 0)} completed installs` : 'No completed installs in this period';
