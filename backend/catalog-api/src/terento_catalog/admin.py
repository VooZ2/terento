...
def test_data_page(
    snapshot: dict[str, Any], user: dict[str, Any], csrf_token: str,
    *, deleted: dict[str, int] | None = None,
) -> bytes:
    compatibility_count = int(snapshot.get("compatibilityEventCount") or 0)
    map_count = int(snapshot.get("mapEventCount") or 0)
    operation_count = int(snapshot.get("operationCount") or 0)
    event_count = compatibility_count + map_count
    last_occurred = snapshot.get("lastOccurredAt")
    notice = ""
    if deleted is not None:
        notice = (
            "<p class='status-success' role='status'>Deleted "
            f"{int(deleted.get('eventCount') or 0)} local test event(s): "
            f"{int(deleted.get('compatibilityEventCount') or 0)} diagnostics and "
            f"{int(deleted.get('mapEventCount') or 0)} map-statistics events.</p>"
        )
    if event_count:
        action = (
            "<form method='post' action='/admin/test-data/delete' "
            "class='admin-async-action' "
            "data-confirm='Delete all local test diagnostics and map statistics? This cannot be undone.'>"
            f"<input type='hidden' name='csrf_token' value='{html.escape(csrf_token, quote=True)}'>"
            "<button type='submit'>Delete local test data</button></form>"
        )
    else:
        action = "<p class='table-help'>There is no classified local test data to delete.</p>"
    content = f"""
      {_admin_header(user, csrf_token, active='test-data')}
      <main class='dashboard' id='main-content'>
        <div class='heading-row'><div><p class='eyebrow'>Data maintenance</p><h1>Local test data</h1><p class='lede'>Use a local build to verify that events arrive, then remove those events before treating the statistics as production evidence.</p></div></div>
        {notice}
        <section class='admin-kpi-grid' aria-label='Local test data summary'>
          <article><span>Test operations</span><strong>{operation_count}</strong></article>
          <article><span>Compatibility diagnostics</span><strong>{compatibility_count}</strong></article>
          <article><span>Map statistics events</span><strong>{map_count}</strong></article>
          <article><span>Latest event</span><strong>{_timestamp_markup(last_occurred)}</strong></article>
        </section>
        <section class='provider-card' aria-labelledby='local-test-data-policy'>
          <div class='section-heading'><div><p class='section-kicker'>Safe boundary</p><h2 id='local-test-data-policy'>What can be deleted</h2></div></div>
          <p>Only rows classified by the server from a release label ending in <code>-local</code> are eligible. Production builds, missing labels, and ordinary diagnostics are not touched. The action deletes both telemetry streams in one transaction and records an admin audit entry.</p>
          {action}
        </section>
      </main>
    """
    return _layout("Local test data", content)
