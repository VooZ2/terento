...
for path in home_files:
    ...
    assert html.count('data-provider-card=') == 3, f"{path}: expected two provider cards plus the optional contour add-on"
    assert html.count('class="provider-benefits"') == 3, f"{path}: expected one benefits list per map card"
