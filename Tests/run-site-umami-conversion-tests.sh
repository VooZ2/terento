...
for path in home_files:
    ...
    assert html.count('data-provider-card=') == 2, f"{path}: expected two provider cards with a nested optional add-on"
    assert html.count('class="provider-benefits"') == 2, f"{path}: expected one benefits list per map card"
