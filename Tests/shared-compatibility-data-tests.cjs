...
assert.doesNotMatch(compatibilityPage, /More models ready for testing/);
assert.doesNotMatch(compatibilityPage, /Evidence refreshed/);
...
for (const locale of ["", "cs/", "de/", "fr/", "it/", "pl/"]) {
  ...
  for (const asset of ["compatibility-data", "compatibility"]) {
    assert.ok(page.includes(`/compatibility/${asset}.js?v=20260914-device-labels-v1`));
  }
}

assert.equal(exactVariantLabel({model: "fēnix 8 Pro", caseSizeMm: 51, screenTechnology: "AMOLED", inReach: true}), "51 mm, AMOLED, inReach");
assert.equal(publicModelName("fēnix 9 Pro · inReach"), "fēnix 9 Pro");

for (const test of JSON.parse(fs.readFileSync(path.join(__dirname, '../contracts/fixtures/device-display-labels.json'), 'utf8'))) {
  const original = JSON.stringify(test.row);
  assert.equal(publicModelName(test.row.model), test.model);
  assert.equal(exactVariantLabel(test.row), test.variant);
  assert.equal(JSON.stringify(test.row), original, 'formatting preserves identity and specification fields');
}
