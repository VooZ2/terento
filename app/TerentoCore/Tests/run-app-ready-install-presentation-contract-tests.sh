...
require_text 'ReadyToInstallSelectedMapsList(plan: plan)' 'Review does not use the bounded selected-map list'
require_text 'plan.selectedPackagePlans' 'Review does not render the selected installation package plans'
require_text 'SelectedMapArtifactSummaryRow' 'Review does not show selected optional map components'
