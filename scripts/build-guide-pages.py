    ...
    graph = [
        ...
        },
        {
            ...
            "itemListElement": [
                ...
                {"@type": "ListItem", "position": 2, "name": copy["breadcrumb_guides"]},
                {"@type": "ListItem", "position": 3, "name": copy["breadcrumb_current"], "item": canonical},
            ],
        },
        {
            "@type": "FAQPage",
...
def faq_html(copy: dict[str, object]) -> str:
    entries = []
    for item in copy["faq"]:
        answer = item["answer"]
        if item.get("support_links"):
            answer = answer.replace(
                "GitHub",
                link("GitHub", ISSUES_URL, external=True),
                1,
            ).replace(
                EMAIL_ADDRESS,
                link(EMAIL_ADDRESS, EMAIL_URL),
                1,
            )
        entries.append(f'<details><summary>{esc(item["question"])}</summary><p>{answer}</p></details>')
    return "".join(entries)
    ...
    <main id="main-content" class="guide-main">
      <article class="guide-article">
        <header class="guide-intro">
          <div class="shell">
            <nav class="breadcrumbs" aria-label="{esc(copy["breadcrumb_label"])}">
              <a href="{home}">{esc(copy["breadcrumb_home"])}</a><span aria-hidden="true">→</span><span>{esc(copy["breadcrumb_guides"])}</span><span aria-hidden="true">→</span><span aria-current="page">{esc(copy["breadcrumb_current"])}</span>
            </nav>
        ...
        <div class="shell guide-content">
          ...
          <section class="guide-faq" id="faq" aria-labelledby="faq-title">
            <div class="guide-section-heading"><p class="eyebrow">{esc(copy["faq_eyebrow"])}</p><h2 id="faq-title">FAQ</h2></div>
            <div class="faq-list">{faq_html(copy)}</div>
          </section>
...
COMMON = {
    "en": {
        ...
        "faq": [
            {"question": "Can I install third-party maps on a Garmin watch from a Mac without BaseCamp?", "answer": "Yes. Terento provides a guided native macOS workflow for installing supported third-party maps. The current beta uses Freizeitkarte and requires an Apple Silicon Mac."},
            {"question": "Why does my Garmin watch not appear in Finder?", "answer": "Many newer Garmin watches use MTP and may not appear as a normal Finder drive. Terento communicates with compatible devices without requiring Finder access."},
            {"question": "How long should Garmin detection take?", "answer": "MTP detection can take 1–2 minutes after connection. Keep the watch connected and close other applications that may already be using it."},
            {"question": "Does Terento replace Garmin Express?", "answer": "No. Terento focuses on installing and managing supported third-party maps. Garmin Express remains relevant for official Garmin device updates and services."},
            {"question": "What should I do if installation fails?", "answer": "Open an issue at GitHub or email the diagnostic log to hello@terento.app. The report helps identify the failure without requiring manual inspection of device folders.", "support_links": True},
        ],
    },
}
...
TRANSLATIONS = {
    "de": {
        ...
        "faq": [{"question": "Kann ich Drittanbieter-Karten auf einer Garmin-Uhr vom Mac aus ohne BaseCamp installieren?", "answer": "Ja. Terento bietet einen geführten nativen macOS-Ablauf zum Installieren unterstützter Drittanbieter-Karten. Die aktuelle Beta verwendet Freizeitkarte und erfordert einen Apple-Silicon-Mac."}, {"question": "Warum erscheint meine Garmin-Uhr nicht im Finder?", "answer": "Viele neuere Garmin-Uhren verwenden MTP und erscheinen möglicherweise nicht als normales Finder-Laufwerk. Terento kommuniziert mit kompatiblen Geräten, ohne dass du Finder-Zugriff brauchst."}, {"question": "Wie lange sollte die Garmin-Erkennung dauern?", "answer": "Die MTP-Erkennung kann nach dem Verbinden 1–2 Minuten dauern. Lass die Uhr verbunden und schließe andere Programme, die sie bereits verwenden könnten."}, {"question": "Ersetzt Terento Garmin Express?", "answer": "Nein. Terento konzentriert sich auf die Installation und Verwaltung unterstützter Drittanbieter-Karten. Garmin Express bleibt für offizielle Garmin-Geräteupdates und Dienste relevant."}, {"question": "Was soll ich tun, wenn die Installation fehlschlägt?", "answer": "Öffne ein Issue auf GitHub oder sende das Diagnoseprotokoll per E-Mail an hello@terento.app. Der Bericht hilft, den Fehler zu erkennen, ohne dass du Geräteordner manuell prüfen musst.", "support_links": True}],
    },
    "fr": {
        "title": "Installer des cartes tierces sur une montre Garmin depuis un Mac — Terento", "h1": "Installer des cartes tierces sur une montre Garmin depuis un Mac", "meta_description": "Installez des cartes tierces sur une montre Garmin compatible depuis un Mac Apple Silicon avec Terento. Connectez la montre, choisissez une région, vérifiez le stockage et installez en toute sécurité.", "eyebrow": "Guide d’installation sur Mac", "intro": "Installer des cartes tierces sur une montre Garmin depuis un Mac peut impliquer le MTP, des transferts de fichiers manuels ou d’anciens outils de bureau. Terento propose un parcours natif Apple Silicon : connectez la montre, choisissez une région, vérifiez le stockage et installez.", "download_beta": "Télécharger la bêta", "see_compatibility": "Voir la compatibilité", "current_beta": "La bêta actuelle utilise Freizeitkarte", "last_reviewed": "Dernière vérification", "reviewed_date": "28 août 2026", "applies_to": "S’applique à", "skip": "Aller au contenu", "breadcrumb_label": "Fil d’Ariane", "breadcrumb_home": "Accueil", "breadcrumb_guides": "Guides", "breadcrumb_current": "Installer des cartes tierces sur Garmin depuis un Mac", "before_eyebrow": "Vérification rapide", "before_title": "Avant de commencer", "checklist": ["Mac Apple Silicon", "macOS 13 ou version ultérieure", "Montre Garmin compatible avec les cartes", "Câble USB permettant le transfert de données", "Dernière bêta de Terento", "Suffisamment d’espace libre sur la montre"], "compatibility_note": "La page Compatibilité présente les modèles et variantes exacts pour lesquels nous disposons de preuves d’installation réelles. Ce n’est pas la liste complète de toutes les montres Garmin susceptibles de fonctionner. Après la connexion, Terento vérifie l’appareil détecté et vous indique si l’installation peut continuer en toute sécurité.", "steps_eyebrow": "Le parcours", "steps_title": "Installer en cinq étapes", "after_steps_label": "Étape suivante", "after_steps_copy": "La montre reste protégée pendant que Terento vérifie la source, le stockage, le transfert et le résultat.", "troubleshooting_eyebrow": "Besoin d’aide ?", "troubleshooting_title": "Si quelque chose ne fonctionne pas", "open_issue": "Ouvrir une issue", "email_log": "Envoyer le journal", "faq_eyebrow": "Questions fréquentes", "bottom_eyebrow": "Prêt quand vous l’êtes", "bottom_title": "Installez la carte dont vous avez besoin sur votre montre.",
        ...
        "faq": [{"question": "Puis-je installer des cartes tierces sur une montre Garmin depuis un Mac sans BaseCamp ?", "answer": "Oui. Terento propose un parcours macOS natif guidé pour installer des cartes tierces prises en charge. La bêta actuelle utilise Freizeitkarte et nécessite un Mac Apple Silicon."}, {"question": "Pourquoi ma montre Garmin n’apparaît-elle pas dans le Finder ?", "answer": "De nombreuses montres Garmin récentes utilisent le MTP et peuvent ne pas apparaître comme un disque Finder classique. Terento communique avec les appareils compatibles sans nécessiter l’accès au Finder."}, {"question": "Combien de temps doit prendre la détection Garmin ?", "answer": "La détection MTP peut prendre 1 à 2 minutes après la connexion. Gardez la montre connectée et fermez les autres apps qui pourraient déjà l’utiliser."}, {"question": "Terento remplace-t-il Garmin Express ?", "answer": "Non. Terento se concentre sur l’installation et la gestion de cartes tierces prises en charge. Garmin Express reste utile pour les mises à jour officielles des appareils et les services Garmin."}, {"question": "Que faire si l’installation échoue ?", "answer": "Ouvrez une issue sur GitHub ou envoyez le journal de diagnostic à hello@terento.app. Le rapport aide à identifier l’échec sans inspection manuelle des dossiers de l’appareil.", "support_links": True}],
    },
    "pl": {
        "title": "Jak instalować mapy innych firm na zegarku Garmin z Maca — Terento", "h1": "Jak instalować mapy innych firm na zegarku Garmin z Maca", "meta_description": "Instaluj mapy innych firm na zgodnym zegarku Garmin z Maca z Apple Silicon za pomocą Terento. Podłącz zegarek, wybierz region, sprawdź pamięć i bezpiecznie zainstaluj mapę.", "eyebrow": "Instrukcja instalacji na Macu", "intro": "Instalowanie map innych firm na zegarku Garmin z Maca może wymagać MTP, ręcznego przesyłania plików lub starszych programów komputerowych. Terento oferuje natywny dla Apple Silicon sposób: podłącz zegarek, wybierz region, sprawdź pamięć i zainstaluj.", "download_beta": "Pobierz betę", "see_compatibility": "Zobacz kompatybilność", "current_beta": "Obecna beta korzysta z Freizeitkarte", "last_reviewed": "Ostatni przegląd", "reviewed_date": "28 sierpnia 2026", "applies_to": "Dotyczy", "skip": "Przejdź do treści", "breadcrumb_label": "Okruszki nawigacji", "breadcrumb_home": "Strona główna", "breadcrumb_guides": "Instrukcje", "breadcrumb_current": "Instalowanie map innych firm na Garminie z Maca", "before_eyebrow": "Szybka kontrola", "before_title": "Zanim zaczniesz", "checklist": ["Mac z Apple Silicon", "macOS 13 lub nowszy", "Zegarek Garmin z obsługą map", "Kabel USB obsługujący przesyłanie danych", "Najnowsza beta Terento", "Wystarczająca ilość wolnego miejsca na zegarku"], "compatibility_note": "Strona kompatybilności pokazuje konkretne modele i warianty z rzeczywistymi dowodami instalacji. Nie jest pełną listą wszystkich zegarków Garmin, które mogą działać. Po podłączeniu Terento sprawdzi wykryte urządzenie i powie, czy instalację można bezpiecznie kontynuować.", "steps_eyebrow": "Przebieg instalacji", "steps_title": "Zainstaluj w pięciu krokach", "after_steps_label": "Następny krok", "after_steps_copy": "Zegarek pozostaje chroniony, gdy Terento sprawdza źródło, pamięć, przesyłanie i wynik.", "troubleshooting_eyebrow": "Potrzebujesz pomocy?", "troubleshooting_title": "Jeśli coś nie działa", "open_issue": "Otwórz zgłoszenie", "email_log": "Wyślij log e-mailem", "faq_eyebrow": "Częste pytania", "bottom_eyebrow": "Gotowe, gdy Ty będziesz", "bottom_title": "Zainstaluj na zegarku mapę, której potrzebujesz.",
        ...
        "faq": [{"question": "Czy mogę instalować mapy innych firm na zegarku Garmin z Maca bez BaseCamp?", "answer": "Tak. Terento oferuje prowadzony, natywny dla macOS sposób instalowania obsługiwanych map innych firm. Obecna beta korzysta z Freizeitkarte i wymaga Maca z Apple Silicon."}, {"question": "Dlaczego mój zegarek Garmin nie pojawia się w Finderze?", "answer": "Wiele nowszych zegarków Garmin korzysta z MTP i może nie pojawiać się jako zwykły dysk w Finderze. Terento komunikuje się ze zgodnymi urządzeniami bez dostępu przez Finder."}, {"question": "Ile czasu powinno zająć wykrycie Garmina?", "answer": "Wykrywanie MTP może potrwać 1–2 minuty po podłączeniu. Pozostaw zegarek podłączony i zamknij inne aplikacje, które mogą już z niego korzystać."}, {"question": "Czy Terento zastępuje Garmin Express?", "answer": "Nie. Terento skupia się na instalowaniu i zarządzaniu obsługiwanymi mapami innych firm. Garmin Express pozostaje potrzebny do oficjalnych aktualizacji urządzeń Garmin i powiązanych usług."}, {"question": "Co zrobić, jeśli instalacja się nie powiedzie?", "answer": "Otwórz zgłoszenie na GitHubie albo wyślij log diagnostyczny na adres hello@terento.app. Raport pomaga ustalić przyczynę bez ręcznego przeglądania folderów urządzenia.", "support_links": True}],
    },
    "cs": {
        "title": "Jak nainstalovat mapy třetích stran do hodinek Garmin z Macu — Terento", "h1": "Jak nainstalovat mapy třetích stran do hodinek Garmin z Macu", "meta_description": "Nainstalujte mapy třetích stran do kompatibilních hodinek Garmin z Macu s Apple Silicon pomocí Terento. Připojte hodinky, vyberte region, zkontrolujte úložiště a bezpečně instalujte.", "eyebrow": "Průvodce instalací na Macu", "intro": "Instalace map třetích stran do hodinek Garmin z Macu může vyžadovat MTP, ruční přenos souborů nebo starší desktopové nástroje. Terento nabízí nativní postup pro Apple Silicon: připojte hodinky, vyberte region, zkontrolujte úložiště a instalujte.", "download_beta": "Stáhnout betu", "see_compatibility": "Zobrazit kompatibilitu", "current_beta": "Aktuální beta používá Freizeitkarte", "last_reviewed": "Naposledy ověřeno", "reviewed_date": "28. srpna 2026", "applies_to": "Platí pro", "skip": "Přejít k obsahu", "breadcrumb_label": "Drobečková navigace", "breadcrumb_home": "Domů", "breadcrumb_guides": "Průvodci", "breadcrumb_current": "Instalace map třetích stran do Garminu z Macu", "before_eyebrow": "Rychlá kontrola", "before_title": "Než začnete", "checklist": ["Mac s Apple Silicon", "macOS 13 nebo novější", "Hodinky Garmin s podporou map", "USB kabel s podporou přenosu dat", "Nejnovější beta Terento", "Dostatek volného místa v hodinkách"], "compatibility_note": "Stránka Kompatibilita zobrazuje přesné modely a varianty s reálnými doklady instalace. Nejde o úplný seznam všech hodinek Garmin, které mohou fungovat. Po připojení Terento zkontroluje rozpoznané zařízení a řekne vám, zda lze bezpečně pokračovat v instalaci.", "steps_eyebrow": "Postup", "steps_title": "Instalace v pěti krocích", "after_steps_label": "Další krok", "after_steps_copy": "Hodinky zůstanou chráněné, zatímco Terento ověří zdroj, úložiště, přenos a výsledek.", "troubleshooting_eyebrow": "Potřebujete pomoc?", "troubleshooting_title": "Když něco nefunguje", "open_issue": "Otevřít issue", "email_log": "Poslat log e-mailem", "faq_eyebrow": "Časté otázky", "bottom_eyebrow": "Připraveno, až budete vy", "bottom_title": "Dostaňte do hodinek mapu, kterou potřebujete.",
        ...
        "faq": [{"question": "Mohu do hodinek Garmin nainstalovat mapy třetích stran z Macu bez BaseCamp?", "answer": "Ano. Terento nabízí řízený nativní postup pro macOS k instalaci podporovaných map třetích stran. Aktuální beta používá Freizeitkarte a vyžaduje Mac s Apple Silicon."}, {"question": "Proč se moje hodinky Garmin nezobrazují ve Finderu?", "answer": "Mnoho novějších hodinek Garmin používá MTP a nemusí se zobrazit jako běžný disk ve Finderu. Terento s kompatibilními zařízeními komunikuje bez přístupu přes Finder."}, {"question": "Jak dlouho má rozpoznání Garminu trvat?", "answer": "Rozpoznání MTP může po připojení trvat 1–2 minuty. Nechte hodinky připojené a zavřete ostatní aplikace, které je mohou používat."}, {"question": "Nahrazuje Terento Garmin Express?", "answer": "Ne. Terento se zaměřuje na instalaci a správu podporovaných map třetích stran. Garmin Express zůstává důležitý pro oficiální aktualizace zařízení Garmin a související služby."}, {"question": "Co mám dělat, když instalace selže?", "answer": "Otevřete issue na GitHubu nebo pošlete diagnostický log na hello@terento.app. Report pomůže určit příčinu bez ruční kontroly složek zařízení.", "support_links": True}],
    },
    "it": {
        "title": "Come installare mappe di terze parti su uno smartwatch Garmin da Mac — Terento", "h1": "Come installare mappe di terze parti su uno smartwatch Garmin da Mac", "meta_description": "Installa mappe di terze parti su uno smartwatch Garmin compatibile da un Mac Apple Silicon con Terento. Collega l’orologio, scegli una regione, controlla lo spazio e installa in sicurezza.", "eyebrow": "Guida all’installazione su Mac", "intro": "Installare mappe di terze parti su uno smartwatch Garmin da Mac può richiedere MTP, trasferimenti manuali dei file o strumenti desktop meno recenti. Terento offre un flusso nativo per Apple Silicon: collega l’orologio, scegli una regione, controlla lo spazio e installa.", "download_beta": "Scarica la beta", "see_compatibility": "Vedi la compatibilità", "current_beta": "La beta attuale usa Freizeitkarte", "last_reviewed": "Ultima verifica", "reviewed_date": "28 agosto 2026", "applies_to": "Si applica a", "skip": "Vai al contenuto", "breadcrumb_label": "Breadcrumb", "breadcrumb_home": "Home", "breadcrumb_guides": "Guide", "breadcrumb_current": "Installare mappe di terze parti su Garmin da Mac", "before_eyebrow": "Un controllo rapido", "before_title": "Prima di iniziare", "checklist": ["Mac Apple Silicon", "macOS 13 o versioni successive", "Smartwatch Garmin con supporto alle mappe", "Cavo USB che supporti il trasferimento dati", "Ultima beta di Terento", "Spazio libero sufficiente sull’orologio"], "compatibility_note": "La pagina Compatibilità mostra modelli e varianti esatti con prove reali di installazione. Non è un elenco completo di tutti gli orologi Garmin che potrebbero funzionare. Dopo il collegamento, Terento controlla il dispositivo rilevato e ti dice se puoi continuare l’installazione in sicurezza.", "steps_eyebrow": "Il flusso", "steps_title": "Installa in cinque passaggi", "after_steps_label": "Passaggio successivo", "after_steps_copy": "L’orologio resta protetto mentre Terento controlla la fonte, lo spazio, il trasferimento e il risultato.", "troubleshooting_eyebrow": "Serve aiuto?", "troubleshooting_title": "Se qualcosa non funziona", "open_issue": "Apri una issue", "email_log": "Invia il log", "faq_eyebrow": "Domande frequenti", "bottom_eyebrow": "Pronto quando vuoi", "bottom_title": "Installa sul tuo orologio la mappa di cui hai bisogno.",
        ...
        "faq": [{"question": "Posso installare mappe di terze parti su uno smartwatch Garmin da Mac senza BaseCamp?", "answer": "Sì. Terento offre un flusso nativo guidato per macOS per installare mappe di terze parti supportate. La beta attuale usa Freizeitkarte e richiede un Mac Apple Silicon."}, {"question": "Perché il mio smartwatch Garmin non appare nel Finder?", "answer": "Molti smartwatch Garmin più recenti usano MTP e potrebbero non apparire come una normale unità del Finder. Terento comunica con i dispositivi compatibili senza richiedere l’accesso al Finder."}, {"question": "Quanto dovrebbe durare il rilevamento Garmin?", "answer": "Il rilevamento MTP può richiedere 1–2 minuti dopo il collegamento. Lascia l’orologio collegato e chiudi le altre app che potrebbero già utilizzarlo."}, {"question": "Terento sostituisce Garmin Express?", "answer": "No. Terento si concentra sull’installazione e la gestione di mappe di terze parti supportate. Garmin Express resta utile per gli aggiornamenti ufficiali dei dispositivi Garmin e per i relativi servizi."}, {"question": "Cosa devo fare se l’installazione non riesce?", "answer": "Apri una issue su GitHub oppure invia il log diagnostico a hello@terento.app. Il report aiuta a individuare il problema senza dover controllare manualmente le cartelle del dispositivo.", "support_links": True}],
    },
}
