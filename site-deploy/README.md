...
Home content is generated by `scripts/normalize-home-ia.py` from
`scripts/templates/home.html` and `scripts/templates/home-copy.json`; provider
cards retain their copy in the generator. Home and Download copy is static HTML,
with no browser translation pass. `scripts/normalize-public-shell.py` owns the
shared navigation markup and translations. The browser only controls the mobile
menu and updates existing Legal/Privacy navigation when their in-page language
changes. Those pages embed the generated shell translation data.
